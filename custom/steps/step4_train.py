...
from utils.config import endpoint, key, blob_sas_url, AppConfig
...
def render_step4(di_endpoint: str, di_key: str):
    ...
    #  define a unique prefix per model so each training run
    #    lives in its own subfolder and never mixes with previous sessions.
    #    e.g. "custom-model-abc123/" — Azure DI will only see files here.
    blob_prefix = f"{model_id_input}/"
    ...
    if can_train and not st.session_state.training_done:
        if st.button("🚀 Upload & Train", type="primary"):
            ...
            #  prepend blob_prefix to every uploaded filename
            #    so files land in  invoice/<model_id>/invoice.pdf
            #    instead of the container root.
            for i, (fname, pdf_bytes_item) in enumerate(st.session_state.uploaded_files.items()):
                try:
                    upload_to_blob(blob_prefix + fname, pdf_bytes_item)
                    progress.progress(
                    ...
                    st.error(f"Upload failed for {fname}: {e}")
                    st.stop()
            ...
            for i, (fname, anns) in enumerate(st.session_state.annotations.items()):
                ...
                lbl_path    = blob_prefix + fname + ".labels.json"
                try:
                    ...
                    st.error(f"Label upload failed: {e}")
                    st.stop()
            ...
            try:
                upload_to_blob(
                    blob_prefix + "fields.json",
                    json.dumps(fields_data, indent="\t").encode(),
                )
                progress.progress(0.70, text="Uploaded fields.json")
            except Exception as e:
                st.error(f"fields.json upload failed: {e}")
                st.stop()
 
            # 4)  training
            #  pass prefix= to AzureBlobContentSource so Azure DI
            #    only reads files inside this model's subfolder, not the whole
            #    container (which would include all previous training sessions).
            try:
                ...
                request = BuildDocumentModelRequest(
                    ...
                    azure_blob_source=AzureBlobContentSource(
                        container_url=blob_sas_url,
                        prefix=blob_prefix,          #  scopes training to this model's folder only
                    ),
                )
